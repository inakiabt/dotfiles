...
while IFS="," read -r name profile email host sign key gpg_format ssh_public_key
do
  ...
  create_profile "$name" "$profile" "$email" "$host" "$sign" "$key" "$gpg_format" "$ssh_public_key"
done < <(op item list --tags ssh-default-profile --format json | op item get - --fields label=name,label=profile,label=email,label=host,label=sign-commits,label=key,label=key-format,label="public key")
...
do
  ...
  create_profile "$name" "$profile" "$email" "$host" "$sign" "$key" "$gpg_format" "$ssh_public_key"
done < <(op item list --tags ssh-profile --format json | op item get - --fields label=name,label=profile,label=email,label=host,label=sign-commits,label=key,label=key-format,label="public key")
